...
trait Mode {
    ...
    fn suggest_all(&self, options: &BotOptions) -> Vec<Placement>;
...
impl Bot {
    ...
    /// Return every explored root placement in search order. The normal TBP
    /// adapter only needs [`Self::suggest`]'s first recommendation; referee
    /// adapters use this larger list only when a first move would top out.
    pub fn suggest_all(&self) -> Vec<Placement> {
        puffin::profile_function!();
        self.mode.suggest_all(&self.options)
    }
