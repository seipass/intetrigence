...
impl Board {
    ...
    pub fn remove_lines(&mut self, lines: u64) {
        for c in &mut self.cols {
            *c = remove_lines_mask(*c, lines);
        }
    }
}

/// Remove the rows in `lines` from a parallel bit mask.
///
/// Replay metadata uses this for tracking which occupied cells came from
/// garbage while `Board` continues to store only occupancy.
pub fn remove_lines_mask(mut value: u64, lines: u64) -> u64 {
    clear_lines(&mut value, lines);
    value
}
