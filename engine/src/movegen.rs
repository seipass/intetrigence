...
pub fn find_moves(board: &Board, piece: Piece) -> Vec<(Placement, u32)> {
    ...
    // A complete Guideline move search must keep exploring after a soft drop:
    // some SRS notch placements require rotating only after the piece has
    // descended into the stack. The top-state shortcut remains safe only when
    // the stack is low; the search still explores every transition there.
    ...
    if fast_mode {
        for &rotation in &[
            ...
            for x in 0..10 {
                ...
                let mut update_position = update_position(&mut queue, &mut values);
    ...
    while let Some(expand) = queue.pop() {
        ...
        *sds = expand.soft_drops.min(*sds);
        ...
        if let Some(mv) = soft_drop(expand.mv, &collision_map) {
            update_position(mv, expand.soft_drops + 1);
        }
...
fn update_position<'a>(
    ...
    move |target: Placement, soft_drops: u32| {
        // Top-state fast mode seeds every orientation and column. Do not
        // discard states above the stack: a soft drop may be needed before a
        // later SRS kick can enter a notch.
...
fn soft_drop(mut placement: Placement, collision_map: &CollisionMaps) -> Option<Placement> {
    placement.location.y -= 1;
    if collision_map.obstructed(placement.location) {
        return None;
    }
    // A downward movement is the last successful action, so it cannot be a
    // T-spin lock even when the preceding state came from a rotation.
...
mod tests {
    ...
    #[test]
    fn soft_drop_clears_rotation_for_spin_detection() {
        let board = Board::default();
        let maps = CollisionMaps::new(&board, Piece::T);
        ...
        let dropped = soft_drop(placement, &maps).expect("spawn can soft-drop");
        assert_eq!(dropped.spin, Spin::None);
        assert_eq!(dropped.location.y, 18);
    }
}
